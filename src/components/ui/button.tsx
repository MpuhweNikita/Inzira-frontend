  ...
  ({ className, variant = 'primary', size = 'md', isLoading, disabled, children, ...props }, ref) => {
    return (
      <motion.button
        ...
        whileTap={{ scale: 0.97 }}
        className={cn(
          'relative inline-flex items-center justify-center font-semibold rounded-xl transition-all outline-none focus:ring-2 focus:ring-primary-accent/40 disabled:opacity-50 disabled:pointer-events-none',
          {
            // Variants
      ...
      >
        ...
        <span className={cn('inline-flex items-center justify-center gap-2 w-full h-full', isLoading && 'opacity-0 invisible pointer-events-none')}>
          {children}
        </span>
      </motion.button>
    );
